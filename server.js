...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS regions (
    ...
    fill_color TEXT DEFAULT 'rgba(100,100,200,0.3)',
    stroke_color TEXT DEFAULT '#333',
  ...
  CREATE TABLE IF NOT EXISTS routes (
    ...
    points TEXT NOT NULL,
    stroke_color TEXT DEFAULT '#8B4513',
    stroke_width INTEGER DEFAULT 3,
    stroke_dash TEXT DEFAULT '',
    description TEXT DEFAULT '',
    FOREIGN KEY (map_id) REFERENCES maps(id) ON DELETE CASCADE
  );
...
app.post('/api/projects/:projectId/steps', authMiddleware, (req, res) => {
  ...
  if (parent_id) {
    const parent = db.prepare('SELECT id FROM steps WHERE id = ? AND project_id = ?').get(parent_id, projectId);
    if (!parent) return res.status(400).json({ error: '父步骤不存在' });
  }

  const maxOrder = db.prepare('SELECT MAX(sort_order) as max_order FROM steps WHERE project_id = ? AND parent_id IS ?')
    .get(projectId, parent_id || null);
  const sortOrder = (maxOrder?.max_order ?? -1) + 1;

  const result = db.prepare('INSERT INTO steps (project_id, parent_id, name, remark, sort_order) VALUES (?, ?, ?, ?, ?)')
...
// Delete step (cascade: deletes all descendant steps and their attachments)
app.delete('/api/projects/:projectId/steps/:stepId', authMiddleware, (req, res) => {
  ...
  function collectIds(parentId) {
    const children = db.prepare('SELECT id FROM steps WHERE parent_id = ?').all(parentId);
  ...
  const deleteTransaction = db.transaction(() => {
    ...
    const attachments = db.prepare(`SELECT * FROM attachments WHERE step_id IN (${placeholders})`).all();
    for (const a of attachments) {
      const filePath = path.join(uploadDir, a.filename);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
    }
    db.prepare(`DELETE FROM attachments WHERE step_id IN (${placeholders})`).run();

    // Delete all steps in the tree
    db.prepare(`DELETE FROM steps WHERE id IN (${placeholders})`).run();
  });
  ...
  try {
    ...
    console.error('Delete step transaction failed:', err);
...
// ============================================================
// World Map System API
// ============================================================

// --- Worlds CRUD ---
app.get('/api/worlds', authMiddleware, (req, res) => {
  const worlds = db.prepare(`
    SELECT w.*, (SELECT COUNT(*) FROM maps m WHERE m.world_id = w.id) as map_count
...
app.post('/api/worlds', authMiddleware, (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: '世界名称不能为空' });
  const result = db.prepare(
    'INSERT INTO worlds (user_id, name, description, rules) VALUES (?, ?, ?, ?)'
  ).run(req.user.id, name, description || '', rules || '');
  const world = db.prepare('SELECT * FROM worlds WHERE id = ?').get(result.lastInsertRowid);
  res.json(world);
});

app.get('/api/worlds/:id', authMiddleware, (req, res) => {
  const world = db.prepare('SELECT * FROM worlds WHERE id = ? AND user_id = ?')
    .get(req.params.id, req.user.id);
  if (!world) return res.status(404).json({ error: '世界不存在' });
  const maps = db.prepare('SELECT * FROM maps WHERE world_id = ? ORDER BY created_at DESC').all(world.id);
  const locations = db.prepare('SELECT * FROM locations WHERE world_id = ? ORDER BY name').all(world.id);
  res.json({ ...world, maps, locations });
});

app.put('/api/worlds/:id', authMiddleware, (req, res) => {
  const { name, description, rules, timeline, cover_image } = req.body;
  const world = db.prepare('SELECT * FROM worlds WHERE id = ? AND user_id = ?')
    .get(req.params.id, req.user.id);
  if (!world) return res.status(404).json({ error: '世界不存在' });
  db.prepare(`
    UPDATE worlds SET name = COALESCE(?, name), description = COALESCE(?, description),
    rules = COALESCE(?, rules), timeline = COALESCE(?, timeline),
    cover_image = COALESCE(?, cover_image), updated_at = CURRENT_TIMESTAMP WHERE id = ?
  `).run(name, description, rules, timeline, cover_image, req.params.id);
  res.json(db.prepare('SELECT * FROM worlds WHERE id = ?').get(req.params.id));
});

app.delete('/api/worlds/:id', authMiddleware, (req, res) => {
  const world = db.prepare('SELECT * FROM worlds WHERE id = ? AND user_id = ?')
    .get(req.params.id, req.user.id);
  if (!world) return res.status(404).json({ error: '世界不存在' });
...
// --- Maps CRUD ---
app.get('/api/worlds/:worldId/maps', authMiddleware, (req, res) => {
  const world = db.prepare('SELECT * FROM worlds WHERE id = ? AND user_id = ?')
    .get(req.params.worldId, req.user.id);
  if (!world) return res.status(404).json({ error: '世界不存在' });
  const maps = db.prepare('SELECT * FROM maps WHERE world_id = ? ORDER BY created_at DESC').all(req.params.worldId);
  res.json(maps);
});

app.post('/api/worlds/:worldId/maps', authMiddleware, (req, res) => {
  const world = db.prepare('SELECT * FROM worlds WHERE id = ? AND user_id = ?')
    .get(req.params.worldId, req.user.id);
  if (!world) return res.status(404).json({ error: '世界不存在' });
  const { name, description, width, height } = req.body;
  if (!name) return res.status(400).json({ error: '地图名称不能为空' });
  const result = db.prepare(
    'INSERT INTO maps (world_id, name, description, width, height) VALUES (?, ?, ?, ?, ?)'
  ).run(req.params.worldId, name, description || '', width || 4096, height || 4096);
  const mapId = result.lastInsertRowid;
  db.prepare('INSERT INTO map_layers (map_id, name, layer_type, sort_order) VALUES (?, ?, ?, ?)').run(mapId, '地形', 'terrain', 0);
  db.prepare('INSERT INTO map_layers (map_id, name, layer_type, sort_order) VALUES (?, ?, ?, ?)').run(mapId, '政治', 'political', 1);
  db.prepare('INSERT INTO map_layers (map_id, name, layer_type, sort_order) VALUES (?, ?, ?, ?)').run(mapId, '路线', 'routes', 2);
  db.prepare('UPDATE worlds SET updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(req.params.worldId);
  const map = db.prepare('SELECT * FROM maps WHERE id = ?').get(mapId);
  res.json(map);
});

app.get('/api/maps/:id', authMiddleware, (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.id, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const layers = db.prepare('SELECT * FROM map_layers WHERE map_id = ? ORDER BY sort_order').all(map.id);
  const locations = db.prepare('SELECT * FROM locations WHERE map_id = ? ORDER BY name').all(map.id);
  const regions = db.prepare('SELECT * FROM regions WHERE map_id = ? ORDER BY name').all(map.id);
  const routesData = db.prepare('SELECT * FROM routes WHERE map_id = ? ORDER BY name').all(map.id);
  res.json({ ...map, layers, locations, regions, routes: routesData });
});
...
app.put('/api/maps/:id', authMiddleware, (req, res) => {
  ...
  `).get(req.params.id, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const { name, description, background_image, width, height, viewport } = req.body;
  db.prepare(`
    UPDATE maps SET name = COALESCE(?, name), description = COALESCE(?, description),
    background_image = COALESCE(?, background_image), width = COALESCE(?, width),
    height = COALESCE(?, height), viewport = COALESCE(?, viewport),
    updated_at = CURRENT_TIMESTAMP WHERE id = ?
  `).run(name, description, background_image, width, height, viewport, req.params.id);
  db.prepare('UPDATE worlds SET updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(map.world_id);
...
app.post('/api/maps/:id/upload-bg', authMiddleware, upload.single('background'), (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.id, req.user.id);
  ...
  db.prepare('UPDATE maps SET background_image = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
    .run(bgPath, req.params.id);
  res.json({ success: true, background_image: bgPath });
});

// --- Save canvas data (Konva JSON) ---
app.post('/api/maps/:id/save-canvas', authMiddleware, (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.id, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const { canvas_data, viewport, thumbnail } = req.body;
  db.prepare(`
    UPDATE maps SET canvas_data = COALESCE(?, canvas_data),
    viewport = COALESCE(?, viewport), thumbnail = COALESCE(?, thumbnail),
    updated_at = CURRENT_TIMESTAMP WHERE id = ?
  `).run(
...
// --- Layers CRUD ---
app.get('/api/maps/:mapId/layers', authMiddleware, (req, res) => {
  const layers = db.prepare(`
    SELECT ml.* FROM map_layers ml JOIN maps m ON ml.map_id = m.id
    JOIN worlds w ON m.world_id = w.id
    WHERE ml.map_id = ? AND w.user_id = ? ORDER BY ml.sort_order
  `).all(req.params.mapId, req.user.id);
  res.json(layers);
});

app.post('/api/maps/:mapId/layers', authMiddleware, (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.mapId, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const { name, layer_type } = req.body;
  if (!name) return res.status(400).json({ error: '图层名称不能为空' });
  const maxOrder = db.prepare('SELECT MAX(sort_order) as m FROM map_layers WHERE map_id = ?').get(req.params.mapId);
  const result = db.prepare(
    'INSERT INTO map_layers (map_id, name, layer_type, sort_order) VALUES (?, ?, ?, ?)'
  ).run(req.params.mapId, name, layer_type || 'custom', (maxOrder.m || 0) + 1);
  res.json(db.prepare('SELECT * FROM map_layers WHERE id = ?').get(result.lastInsertRowid));
});

app.put('/api/layers/:id', authMiddleware, (req, res) => {
  const { name, visible, sort_order } = req.body;
  db.prepare(`
    UPDATE map_layers SET name = COALESCE(?, name), visible = COALESCE(?, visible),
    sort_order = COALESCE(?, sort_order) WHERE id = ?
  `).run(name, visible, sort_order, req.params.id);
  res.json(db.prepare('SELECT * FROM map_layers WHERE id = ?').get(req.params.id));
});

app.delete('/api/layers/:id', authMiddleware, (req, res) => {
  db.prepare('DELETE FROM map_layers WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});

// --- Locations CRUD ---
app.get('/api/maps/:mapId/locations', authMiddleware, (req, res) => {
  const locations = db.prepare(`
    SELECT l.* FROM locations l JOIN maps m ON l.map_id = m.id
    JOIN worlds w ON m.world_id = w.id
    WHERE l.map_id = ? AND w.user_id = ? ORDER BY l.name
  `).all(req.params.mapId, req.user.id);
  res.json(locations);
});

app.post('/api/maps/:mapId/locations', authMiddleware, (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.mapId, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const { name, type, x, y, icon, color, size, description, lore, layer_id } = req.body;
  if (!name || x == null || y == null) return res.status(400).json({ error: '名称和坐标不能为空' });
  const result = db.prepare(`
    INSERT INTO locations (world_id, map_id, layer_id, name, type, x, y, icon, color, size, description, lore)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(map.world_id, req.params.mapId, layer_id || null, name, type || 'city', x, y,
    icon || 'circle', color || '#ff6600', size || 12, description || '', lore || '');
...
app.put('/api/locations/:id', authMiddleware, (req, res) => {
  const loc = db.prepare(`
    SELECT l.* FROM locations l JOIN worlds w ON l.world_id = w.id
    WHERE l.id = ? AND w.user_id = ?
  `).get(req.params.id, req.user.id);
  if (!loc) return res.status(404).json({ error: '地点不存在' });
  const fields = ['name', 'type', 'x', 'y', 'icon', 'color', 'size', 'description', 'lore', 'population', 'government', 'images', 'metadata'];
  const updates = {};
  for (const f of fields) { if (req.body[f] !== undefined) updates[f] = req.body[f]; }
  if (Object.keys(updates).length === 0) return res.json(loc);
  const setClauses = Object.keys(updates).map(k => `${k} = ?`).join(', ');
  const values = Object.values(updates);
  db.prepare(`UPDATE locations SET ${setClauses}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
...
// --- Regions CRUD ---
app.get('/api/maps/:mapId/regions', authMiddleware, (req, res) => {
  const regions = db.prepare(`
    SELECT r.* FROM regions r JOIN maps m ON r.map_id = m.id
    JOIN worlds w ON m.world_id = w.id
    WHERE r.map_id = ? AND w.user_id = ? ORDER BY r.name
  `).all(req.params.mapId, req.user.id);
  res.json(regions);
});

app.post('/api/maps/:mapId/regions', authMiddleware, (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.mapId, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const { name, type, points, fill_color, stroke_color, stroke_width, description, layer_id } = req.body;
  if (!name || !points) return res.status(400).json({ error: '名称和顶点数据不能为空' });
  const result = db.prepare(`
    INSERT INTO regions (map_id, layer_id, name, type, points, fill_color, stroke_color, stroke_width, description)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.params.mapId, layer_id || null, name, type || 'country',
    JSON.stringify(points), fill_color || 'rgba(100,100,200,0.3)',
    stroke_color || '#333', stroke_width || 2, description || '');
  res.json(db.prepare('SELECT * FROM regions WHERE id = ?').get(result.lastInsertRowid));
});

app.put('/api/regions/:id', authMiddleware, (req, res) => {
  ...
  const { name, type, points, fill_color, stroke_color, stroke_width, description } = req.body;
  db.prepare(`
    UPDATE regions SET name = COALESCE(?, name), type = COALESCE(?, type),
    points = COALESCE(?, points), fill_color = COALESCE(?, fill_color),
    stroke_color = COALESCE(?, stroke_color), stroke_width = COALESCE(?, stroke_width),
    description = COALESCE(?, description) WHERE id = ?
  `).run(name, type, points ? JSON.stringify(points) : null,
    fill_color, stroke_color, stroke_width, description, req.params.id);
  res.json(db.prepare('SELECT * FROM regions WHERE id = ?').get(req.params.id));
});

app.delete('/api/regions/:id', authMiddleware, (req, res) => {
  db.prepare('DELETE FROM regions WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});

// --- Routes CRUD ---
app.get('/api/maps/:mapId/routes', authMiddleware, (req, res) => {
  const routesData = db.prepare(`
    SELECT r.* FROM routes r JOIN maps m ON r.map_id = m.id
    JOIN worlds w ON m.world_id = w.id
    WHERE r.map_id = ? AND w.user_id = ? ORDER BY r.name
  `).all(req.params.mapId, req.user.id);
  res.json(routesData);
...
app.post('/api/maps/:mapId/routes', authMiddleware, (req, res) => {
  const map = db.prepare(`
    SELECT m.* FROM maps m JOIN worlds w ON m.world_id = w.id
    WHERE m.id = ? AND w.user_id = ?
  `).get(req.params.mapId, req.user.id);
  if (!map) return res.status(404).json({ error: '地图不存在' });
  const { name, type, points, stroke_color, stroke_width, stroke_dash, description, layer_id } = req.body;
  if (!name || !points) return res.status(400).json({ error: '名称和路径数据不能为空' });
  const result = db.prepare(`
    INSERT INTO routes (map_id, layer_id, name, type, points, stroke_color, stroke_width, stroke_dash, description)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.params.mapId, layer_id || null, name, type || 'road',
    JSON.stringify(points), stroke_color || '#8B4513', stroke_width || 3,
    stroke_dash || '', description || '');
  res.json(db.prepare('SELECT * FROM routes WHERE id = ?').get(result.lastInsertRowid));
});
...
app.put('/api/routes/:id', authMiddleware, (req, res) => {
  const route = db.prepare(`
    ...
    WHERE r.id = ? AND w.user_id = ?
  `).get(req.params.id, req.user.id);
  if (!route) return res.status(404).json({ error: '路径不存在' });
  const { name, type, points, stroke_color, stroke_width, stroke_dash, description } = req.body;
  db.prepare(`
    UPDATE routes SET name = COALESCE(?, name), type = COALESCE(?, type),
    points = COALESCE(?, points), stroke_color = COALESCE(?, stroke_color),
    stroke_width = COALESCE(?, stroke_width), stroke_dash = COALESCE(?, stroke_dash),
    description = COALESCE(?, description) WHERE id = ?
  `).run(name, type, points ? JSON.stringify(points) : null,
    stroke_color, stroke_width, stroke_dash, description, req.params.id);
...
// --- Location Relations ---
app.get('/api/locations/:id/relations', authMiddleware, (req, res) => {
  const relations = db.prepare(`
    ...
    JOIN worlds w ON l1.world_id = w.id
    WHERE (lr.from_location_id = ? OR lr.to_location_id = ?) AND w.user_id = ?
  `).all(req.params.id, req.params.id, req.user.id);
  res.json(relations);
});

app.post('/api/relations', authMiddleware, (req, res) => {
  const { from_location_id, to_location_id, relation_type, weight, description } = req.body;
  if (!from_location_id || !to_location_id || !relation_type) {
    return res.status(400).json({ error: '缺少必要参数' });
  }
  const result = db.prepare(`
    INSERT INTO location_relations (from_location_id, to_location_id, relation_type, weight, description)
    VALUES (?, ?, ?, ?, ?)
  `).run(from_location_id, to_location_id, relation_type, weight || 1, description || '');
  res.json(db.prepare('SELECT * FROM location_relations WHERE id = ?').get(result.lastInsertRowid));
});

app.delete('/api/relations/:id', authMiddleware, (req, res) => {
  db.prepare('DELETE FROM location_relations WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});

// --- World cover upload ---
app.post('/api/worlds/:id/upload-cover', authMiddleware, upload.single('cover'), (req, res) => {
  const world = db.prepare('SELECT * FROM worlds WHERE id = ? AND user_id = ?')
    .get(req.params.id, req.user.id);
  if (!world) return res.status(404).json({ error: '世界不存在' });
  if (!req.file) return res.status(400).json({ error: '未上传文件' });
  const coverPath = '/uploads/' + req.file.filename;
  db.prepare('UPDATE worlds SET cover_image = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
    .run(coverPath, req.params.id);
